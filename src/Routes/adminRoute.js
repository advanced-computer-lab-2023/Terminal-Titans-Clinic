import express from 'express'
import User from '../Models/userModel.js';
import Admin from '../Models/adminModel.js';
import DoctorApplication from '../Models/requestedDoctorModel.js';
import HealthPackage from '../Models/healthPackageModel.js';

const router = express.Router()

//requirement number 7
router.post('/createAdmin', async (req, res) => {
    // Create the admin
    try {
        const { Username, Password } = req.body;
        const userexist = await Admin.findOne({ Username });
        if (!userexist) {
            const admin = new Admin({ Username, Password });
            const savedAdmin = await admin.save();
            res.status(200).json({
                success:true,
                savedAdmin:savedAdmin});
        }
        else {
            res.status(500).json({ 
                success:false,
                message: 'Username already exist' });
        }
    }
    catch (error) {
        console.error('Error: ', error);
        res.status(500).json({
            success:false,
            message: "General Error" })
    }
});


//requirement number 8
router.delete('/deleteUser/:username', async (req, res) => {
    try {
        const username= req.params.username;
        const user = await User.findOne({Username: username});
        console.log(user)
        if (user) {
            await User.deleteOne(user);
            const users = await User.find({});
            res.status(200).json({
            success:true,
            deletedUser: user,
            usersLeft: users,});
        }
        else {
            res.status(500).json({ 
                success:false,
                message: "User doesn't exist" });
        }
    }
    catch (error) {
        console.error('Error: ', error);
        res.status(500).json({
            success:false,
            message: "General Error" })
    }
});

//requirement number 9
router.get('/viewDoctorApplication/:doctornName', async (req, res) => {
    try {
        const doctorName= req.params.doctornName;
        const doctorApplication = await DoctorApplication.findOne({ Name: new RegExp(`${doctorName}`) });
        if (doctorApplication) {
            res.status(200).json({
                success:true,
                doctorApplication: doctorApplication});
        }
        else {
            res.status(500).json({ 
                success:false,
                message: "No doctor applications found with this name" });
        }
    }
    catch (error) {
        console.error('Error: ', error);
        res.status(500).json({
            success:false,
            message: "General Error" })
    }
});


//requirement number 10
router.post('/addHealthPackage', async (req, res) => {
    const newPackage = req.body.healthPackage;
    const packageType = newPackage.packageType;
    try {
            const existingPackage = await HealthPackage.findOne({packageType:packageType});
            if(existingPackage){
                res.status(500).json({ 
                    success:false,
                    message: "This package already exists" });
            }else{
                const heathPackage = new HealthPackage({
                    packageType : packageType,
                    subsriptionFeesInEGP : newPackage.subsriptionFeesInEGP,
                    doctorDiscountInPercentage : newPackage.doctorDiscountInPercentage,
                    medicinDiscountInPercentage : newPackage.medicinDiscountInPercentage,
                    familyDiscountInPercentage : newPackage.familyDiscountInPercentage
                });
                heathPackage.save();
                res.status(200).json({
                    success:true,
                    healthPackage: heathPackage});
            }
    }
    catch (error) {
        console.error('Error: ', error);
        res.status(500).json({
            success:false,
            message: "General Error" })
    }
});


//requirement number 10
router.put('/updateHealthPackage', async (req, res) => {
    const packageType = req.body.healthPackage.packageType;
    try {
            const existingPackage = await HealthPackage.findOne({packageType:packageType});
            let newPackage = req.body.healthPackage
            if(existingPackage){
                await HealthPackage.findByIdAndUpdate(existingPackage._id,newPackage);
                res.status(200).json({
                    success:true,
                    updatedHealthPackage: newPackage});
            }else{
                res.status(500).json({ 
                    success:false,
                    message: "This package doesn't exist" });
            }
    }
    catch (error) {
        console.error('Error: ', error);
        res.status(500).json({
            success:false,
            message: "General Error" })
    }
});


//requirement number 10
router.delete('/deleteHealthPackage', async (req, res) => {
    const packageType = req.body.packageType;
    try {
            const existingPackage = await HealthPackage.findOne({packageType:packageType});
            if(existingPackage){
                await HealthPackage.deleteOne({packageType:packageType});
                res.status(200).json(
                    {success:true,
                    deletedHealthPackage: existingPackage});
            }else{
                res.status(500).json({
                     success:false,
                     message: "This package doesn't exist" });
            }
    }
    catch (error) {
        console.error('Error: ', error);
        res.status(500).json({
            success:false,
            message: "General Error" })
    }
});

// //requirement number 10
// router.post('/addHealthPackage', async (req, res) => {
//     const patientUsername = req.body.patientUsername;
//     const packageType = req.body.packageType;
//     try {
//         const patient = await Patient.findOne({Username: patientUsername});
//         if (patient) {
//             const existingPackage = await HealthPackage.findOne({patientUsername:patientUsername});
//             if(existingPackage){
//                 res.status(500).json({ message: "Patient is already enrolled in a package" });
//             }else{
//                 const heathPackage = new HealthPackage({
//                     patientUsername : patientUsername,
//                     packageType : packageType,
//                     patientId:patient._id,
//                     enrollmentDate:new Date()
//                 });
//                 heathPackage.save();
//                 res.status(200).json({healthPackage: heathPackage});
//             }
//         }
//         else {
//             res.status(500).json({ message: "Patient Doesn't Exist" });
//         }
//     }
//     catch (error) {
//         console.error('Error: ', error);
//         res.status(500).json({ message: "General Error" })
//     }
// });
// 
// //requirement number 10
// router.put('/updateHealthPackage', async (req, res) => {
//     const patientUsername = req.body.patientUsername;
//     const packageType = req.body.packageType;
//     try {
//         const patient = await Patient.findOne({Username: patientUsername});
//         if (patient) {
//             const existingPackage = await HealthPackage.findOne({patientUsername:patientUsername});
//             if(!existingPackage){
//                 res.status(500).json({ message: "Patient isn't enrolled in a package" });
//             }else if(existingPackage.packageType==packageType){
//                 res.status(500).json({ message: "Patient is already enrolled in this package" });
//             }else{
//                 let newPackage = existingPackage;
//                 newPackage.packageType = packageType;
//                 await HealthPackage.findByIdAndUpdate(existingPackage._id,newPackage);
//                 res.status(200).json({updatedHealthPackage: newPackage});
//             }
//         }
//         else {
//             res.status(500).json({ message: "Patient Doesn't Exist" });
//         }
//     }
//     catch (error) {
//         console.error('Error: ', error);
//         res.status(500).json({ message: "General Error" })
//     }
// });

// //requirement number 10
// router.delete('/deleteHealthPackage', async (req, res) => {
//     const patientUsername = req.body.patientUsername;
//     const packageType = req.body.packageType;
//     try {
//         const patient = await Patient.findOne({Username: patientUsername});
//         if (patient) {
//             const existingPackage = await HealthPackage.findOne({patientUsername:patientUsername});
//             if(existingPackage && existingPackage.packageType==packageType){
//                 await HealthPackage.deleteOne({patientUsername:patientUsername});
//                 res.status(200).json({deletedHealthPackage: existingPackage});
//             }else{
//                 res.status(500).json({ message: "Patient isn't enrolled in this package" });
//             }
//         }
//         else {
//             res.status(500).json({ message: "Patient Doesn't Exist" });
//         }
//     }
//     catch (error) {
//         console.error('Error: ', error);
//         res.status(500).json({ message: "General Error" })
//     }
// });

export default router;