...
const cancelfunc=async()=>{
   console.log(aptmnts[curId])
    const response = await axios.put(
        `http://localhost:8000/doctor/cancelAppointment/${aptmnts[curId]?.id}`,
        {},
        { headers: { Authorization: 'Bearer ' + sessionStorage.getItem("token") } }
      );
      if (response.status === 200) {
        ...
        <form>
                     <label style={{display:'inline-block',width:'50%',fontSize:'18px'}}  >Patient Name:</label> 
                        ...
                        <>
                         ...
                         <Button variant="dark" style={{ width: '100%' }}onClick={() => window.location.href=`/Health-Plus/followUpDoc?Id=${aptmnts[curId]?.id}`}>
                        ...
                        <Button variant="dark" style={{ width: '48%',marginRight:'4%',marginTop:'4%' }} onClick={() => window.location.href=`/Health-Plus/reschduleDoc?Id=${aptmnts[curId]?.id}`}>
                        reschedule</Button>
