...
function DoctorRegistrationForm() {
  ...
  const [isSpecialityInvalid, setIsSpecialityInvalid] = React.useState(false);
  ...
  const handleFile2Change = (e) => {
    document.getElementById('degBtn').style.backgroundColor = '#4caf50';
  ...
  const handleSubmit = async (event) => {
    ...
    if (flag) {
      ...
      formData.append('speciality', mySpec);
      formData.append('ID', iD)
      formData.append('Degree', degree);
      formData.append('License', license);
      try {
        let response = await axios.post('http://localhost:8000/security/doctor', formData).then((response) => {
  ...
  return (
    <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 3 }}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={4}>
          <Button component="label" variant="contained" startIcon={<CloudUploadIcon />} fullWidth id='degBtn'>
            Upload Degree
